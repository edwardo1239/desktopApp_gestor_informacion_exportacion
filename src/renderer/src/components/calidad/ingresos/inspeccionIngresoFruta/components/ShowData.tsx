...
export default function ShowData(props: propsType): JSX.Element {
    ...
    const [totalUnidades, setToralUnidades] = useState<number>()
    useEffect(() => {

        const total = props.dataArray.reduce((acu, item) => acu += item.lavado, 0);
        if (total === 0) {
            setData([])
            return;
        }
        const porcentages: elementoPorcentajeType[] = props.dataArray.map(item => {
            ...
            return { defecto: item.defecto, porcentage: porcentage, unidades: totalDefecto }
        });
        setData(porcentages)
        setToralUnidades(total)
    }, [props.dataArray]);
    ...
    return (
        <div className="container-clasificacion-calidad-show-info">
            {data.map((item, index) => (
                <div key={index} className="container-clasificacion-calidad-show-info-item">
                    <div className="container-clasificacion-calidad-show-info-item-defecto">
                        <p>{
                            props.dataFormulario && props.dataFormulario[item.defecto].name}
                        </p>
                        <p>-</p>
                        <p>{item.unidades} Unidades</p>
                        <p>-</p>
            ...
            <div>Total unidades: {totalUnidades} unidades</div>
        </div>
    )
}